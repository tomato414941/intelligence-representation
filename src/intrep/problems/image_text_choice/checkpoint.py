...
from intrep.domains.language.tokenizer import TextTokenizer, text_tokenizer_from_payload, text_tokenizer_to_payload
...
def save_image_text_choice_checkpoint(path: str | Path, result: ImageTextChoiceTrainingResult) -> None:
    checkpoint_path = Path(path)
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "schema_version": "intrep.image_text_choice_checkpoint.v1",
...
def load_image_text_choice_checkpoint(
    path: str | Path,
    *,
    device: TrainingDevice = "auto",
) -> ImageTextChoiceCheckpoint:
    ...
    if payload.get("schema_version") != "intrep.image_text_choice_checkpoint.v1":
        raise ValueError("unsupported image/text choice checkpoint schema")
    ...
    if not isinstance(metrics, dict):
        metrics = {}
    return ImageTextChoiceCheckpoint(
        model=model,
        tokenizer=tokenizer,
        config=config,
