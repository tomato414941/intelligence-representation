...
class ImageTextChoiceDatasetBuild:
...
def write_idx_image_text_choice_jsonl(
    *,
    ...
    _write_jsonl(output_path, [image_text_choice_example_to_record(example) for example in examples])
    return ImageTextChoiceDatasetBuild(examples=examples, image_count=count, output_dir=output_dir)
...
def write_cifar10_image_text_choice_jsonl(
    *,
    ...
    _write_jsonl(output_path, [image_text_choice_example_to_record(example) for example in examples])
    return ImageTextChoiceDatasetBuild(examples=examples, image_count=len(examples), output_dir=output_dir)
