...
class ShogiDlshogiLikePositionInputLayer(nn.Module):
    def __init__(self, *, embedding_dim: int) -> None:
        ...
        self.global_element_embedding = nn.Embedding(SHOGI_DLSHOGI_LIKE_GLOBAL_ELEMENT_COUNT, embedding_dim)
        self.square_field_embedding = nn.Embedding(SHOGI_DLSHOGI_LIKE_SQUARE_FEATURE_COUNT, embedding_dim)
        self.square_position_embedding = nn.Embedding(SHOGI_DLSHOGI_LIKE_SQUARE_ELEMENT_COUNT, embedding_dim)
    ...
    def _global_embeddings(self, position_features: ShogiPositionFeatures) -> torch.Tensor:
        ...
        global_elements = torch.arange(SHOGI_DLSHOGI_LIKE_GLOBAL_ELEMENT_COUNT, device=global_feature_ids.device).unsqueeze(
            0
        )
    ...
    def _square_embeddings(self, position_features: ShogiPositionFeatures) -> torch.Tensor:
        ...
        square_positions = torch.arange(SHOGI_DLSHOGI_LIKE_SQUARE_ELEMENT_COUNT, device=square_feature_ids.device).unsqueeze(
            0
        )
