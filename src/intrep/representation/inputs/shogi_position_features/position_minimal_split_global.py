...
SHOGI_MINIMAL_SPLIT_GLOBAL_SQUARE_FEATURE_COUNT = 1
...
def validate_shogi_minimal_split_global_position_feature_structure(features: ShogiPositionFeatures) -> None:
    ...
    _validate_integer_tensor_shape(
        ...
        (SHOGI_MINIMAL_SPLIT_GLOBAL_SQUARE_ELEMENT_COUNT, SHOGI_MINIMAL_SPLIT_GLOBAL_SQUARE_FEATURE_COUNT),
    )
    ...
    if int(features.pair_relation_edges.relation_ids.numel()) != 0:
        raise ValueError("minimal-split-global position features must not contain pair relation edges")
