...
from intrep.representation.inputs.shogi_position_features.position_minimal_global import (
    ...
    SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT,
...
class ShogiMinimalGlobalPositionInputLayer(nn.Module):
    def __init__(self, *, embedding_dim: int) -> None:
        super().__init__()
        self.feature_embedding = nn.Embedding(SHOGI_POSITION_FEATURE_VOCAB_SIZE, embedding_dim)
        self.global_feature_embedding = nn.Embedding(SHOGI_MINIMAL_GLOBAL_GLOBAL_FEATURE_COUNT, embedding_dim)
        self.square_slot_embedding = nn.Embedding(SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT, embedding_dim)
    ...
    def _global_embedding(self, position_features: ShogiPositionFeatures) -> torch.Tensor:
        ...
        feature_slots = torch.arange(SHOGI_MINIMAL_GLOBAL_GLOBAL_FEATURE_COUNT, device=global_feature_ids.device)
        hidden = (
            self.feature_embedding(global_feature_ids)
            + self.global_feature_embedding(feature_slots).view(1, 1, SHOGI_MINIMAL_GLOBAL_GLOBAL_FEATURE_COUNT, -1)
        ).sum(dim=2)
    ...
    def _square_embeddings(self, position_features: ShogiPositionFeatures) -> torch.Tensor:
        ...
        square_slots = torch.arange(SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT, device=square_feature_ids.device).unsqueeze(
            0
        )
        return self.square_norm(self.feature_embedding(square_feature_ids) + self.square_slot_embedding(square_slots))


class ShogiMinimalGlobalPositionAttentionLogitBias(nn.Module):
    ...
    def forward(self, position_features: ShogiPositionFeatures, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        if embeddings.size(1) != SHOGI_MINIMAL_GLOBAL_ELEMENT_COUNT:
            raise ValueError("embeddings must use the minimal-global shogi position length")
        bias = torch.zeros(
            (SHOGI_MINIMAL_GLOBAL_ELEMENT_COUNT, SHOGI_MINIMAL_GLOBAL_ELEMENT_COUNT),
            device=embeddings.device,
            dtype=embeddings.dtype,
        )
        square_bias = self.square_relation_bias(self.square_relation_ids.to(embeddings.device)).squeeze(-1)
        square_start = SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_OFFSET
        square_end = square_start + SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT
...
class ShogiMinimalGlobalPositionEncoder(nn.Module):
    def __init__(
        self,
        *,
...
def _square_geometry_relation_ids() -> torch.Tensor:
    relation_ids = torch.empty(
        (SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT, SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT),
        dtype=torch.long,
    )
    for from_square in range(SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT):
        from_file = from_square % 9
        from_rank = from_square // 9
        for to_square in range(SHOGI_MINIMAL_GLOBAL_SQUARE_ELEMENT_COUNT):
            to_file = to_square % 9
            to_rank = to_square // 9
