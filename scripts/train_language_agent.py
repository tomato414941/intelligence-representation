...
def main():
    ...
    if not args.skip_evaluation:
        native = read_native_base(args.native_checkpoint)
        model = LanguageAgentModel(MultimodalAgentConfig(**native['config'])).to(args.device)
    ...
    path = train([args.selection], args.conversations, args.output / 'learning', config, device=args.device,
                 native_checkpoint=args.native_checkpoint)
    gc.collect()
    torch.cuda.empty_cache()
    if not args.skip_evaluation:
        model, payload = load_checkpoint(path, device=args.device)
