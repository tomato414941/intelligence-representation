...
from intrep.problems.shared_prediction.rule_transfer_training import (
    LESSON_KEY, LESSON_NAMES, RuleLessons, measure_prerequisites, state_digest, validate_training_recipe,
)
...
def train_trial(args):
    ...
    if (args.manifest is None) != (args.condition == "calibration"):
    ...
    weights.update({name: args.weights[LESSON_NAMES.index(name)] for name in lessons.names})
    ...
    if args.resume:
        ...
        if args.common:
            if (old_metadata is None or old_metadata["settings"]["condition"] != "calibration"
                    or not old_metadata.get("prerequisites_passed")):
                raise ValueError("interventions require a common checkpoint that passed calibration")
            ...
            lessons.load_state_dict(payload["sources"][LESSON_KEY])
        metadata = {"schema_version": "intrep.rule_transfer_training.v1", "settings": settings,
    ...
    def measure():
        result = measure_prerequisites(readout_source, panel, images, labels, condition=args.condition,
        ...
        print(json.dumps({"stage": "prerequisites", "step": trainer.steps, "gates": result["gates"]}), flush=True)
        return result
    ...
    try:
        while trainer.steps < args.steps:
            ...
            if trainer.steps % args.interval == 0:
                measured = measure()
                save()
    finally:
        if audit:
