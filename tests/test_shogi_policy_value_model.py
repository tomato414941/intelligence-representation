...
class ShogiPolicyValueModelTest(unittest.TestCase):
    ...
    def test_minimal_global_policy_plane_assembly_spec_uses_minimal_global_input(self) -> None:
        spec = shogi_policy_value_assembly_spec_for_id(SHOGI_POLICY_VALUE_MINIMAL_GLOBAL_POLICY_PLANE_ASSEMBLY_SPEC_ID)

        self.assertEqual(spec["input"], SHOGI_MINIMAL_GLOBAL_POSITION_INPUT_MODULE_ID)
        self.assertEqual(spec["policy_output"], SHOGI_POLICY_PLANE_OUTPUT_MODULE_ID)
    ...
    def test_minimal_global_policy_plane_model_uses_minimal_global_position_length(self) -> None:
        position_features = stack_shogi_position_features(
            [
                shogi_minimal_global_position_features_from_sfen("lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL b - 1"),
                shogi_minimal_global_position_features_from_sfen("lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL b - 1"),
            ]
        )
        legal_action_mask = torch.ones((2, SHOGI_POLICY_PLANE_ACTION_COUNT), dtype=torch.bool)
