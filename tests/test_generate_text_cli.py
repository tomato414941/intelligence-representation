...
from intrep.sources.language.tokenizer import train_byte_pair_tokenizer, text_tokenizer_to_payload
...
class GenerateTextCLITest(unittest.TestCase):
    ...
    def test_generates_text_from_byte_pair_checkpoint(self) -> None:
        with TemporaryDirectory() as directory:
            checkpoint_path = Path(directory) / "checkpoint.pt"
            tokenizer = train_byte_pair_tokenizer("hello hello", vocab_size=270)
            _write_checkpoint(
                checkpoint_path,
