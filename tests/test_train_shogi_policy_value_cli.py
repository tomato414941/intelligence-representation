...
import intrep.problems.shogi_policy_value.tensor_cache as tensor_cache_module
...
class TrainShogiPolicyValueCliTest(unittest.TestCase):
    def test_trains_from_game_records_and_writes_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("initial_train_eval start batches=", output)
            self.assertIn("initial_train_eval batch=", output)
            self.assertIn("initial_train_eval done batches=", output)
            self.assertIn("initial_eval start batches=", output)
            self.assertIn("initial_eval batch=", output)
            self.assertIn("initial_eval done batches=", output)
    ...
    def test_action_plane_policy_tensor_cache_converts_shard_samples_lazily(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            train_games_path = root / "train-games.jsonl"
            eval_games_path = root / "eval-games.jsonl"
            ...
            write_shogi_game_records_jsonl(
                train_games_path,
                [_record(("7g7f", "3c3d", "2g2f", "8c8d"), "black")],
            )
            write_shogi_game_records_jsonl(eval_games_path, [_record(("7g7f", "3c3d"), "white")])
            data_selection_path.write_text(
                json.dumps(
                    {
                        "name": "test-shogi-action-plane-policy-lazy-cache",
                        "objective": "shogi policy-value",
                        "target_construction": {
                            "policy": "chosen_move",
                            "policy_temperature_cp": 100.0,
    ...
    def test_tensor_cache_rejects_output_space_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            eval_games_path = root / "eval-games.jsonl"
            data_selection_path = root / "data-selection.json"
            tensor_cache_path = root / "cache" / "legal-move"
            write_shogi_game_records_jsonl(train_games_path, [_record(("7g7f", "3c3d"), "white")])
            write_shogi_game_records_jsonl(eval_games_path, [_record(("2g2f", "8c8d"), "black")])
            data_selection_path.write_text(
                json.dumps(
    ...
    def test_tensor_cache_identity_survives_bundle_relocation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            tensor_cache_path = source_root / "cache" / "legal-move"
            write_shogi_game_records_jsonl(train_games_path, [_record(("7g7f", "3c3d"), "white")])
            write_shogi_game_records_jsonl(eval_games_path, [_record(("2g2f", "8c8d"), "black")])
            data_selection_path.write_text(
                json.dumps(
            ...
            shutil.copytree(tensor_cache_path, moved_root / "cache" / "legal-move")
