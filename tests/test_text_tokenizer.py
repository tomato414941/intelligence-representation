...
from intrep.text_tokenizer import (
    ...
    train_huggingface_byte_pair_tokenizer,
)
...
class TextTokenizerTest(unittest.TestCase):
    ...
    def test_huggingface_byte_pair_tokenizer_round_trips_unseen_text(self) -> None:
        tokenizer = train_huggingface_byte_pair_tokenizer("hello hello 箱 hello", vocab_size=270)
    ...
    def test_saves_and_loads_huggingface_text_tokenizer_file(self) -> None:
        with TemporaryDirectory() as directory:
            path = Path(directory) / "tokenizer.json"
            original = train_huggingface_byte_pair_tokenizer("hello hello", vocab_size=270)

            save_text_tokenizer(path, original)
            restored = load_text_tokenizer(path)

        self.assertIsInstance(restored, HuggingFaceBytePairTokenizer)
