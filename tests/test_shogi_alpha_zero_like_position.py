...
class ShogiAlphaZeroLikePositionTest(unittest.TestCase):
    def test_manifest_matches_alpha_zero_like_current_state_intent(self) -> None:
        self.assertEqual(
            SHOGI_ALPHA_ZERO_LIKE_POSITION_FEATURE_MANIFEST_HASH,
            shogi_alpha_zero_like_position_feature_manifest_hash(),
        )
    ...
    def test_global_features_are_state_side_move_count_and_hands(self) -> None:
        ...
        features = shogi_alpha_zero_like_position_features_from_sfen(board.sfen())
    ...
    def test_white_to_move_uses_relative_piece_planes_and_hands(self) -> None:
        features = shogi_alpha_zero_like_position_features_from_sfen("4k4/9/9/9/4+R4/9/9/9/4K4 w 2P3p2B 221")
        ...
        self.assertEqual(int(features.global_feature_ids[opponent_bishop_hand_index].item()), OPPONENT_HAND_OFFSET + 2)
        self.assertEqual(
            int(features.square_feature_ids[relative_5a, shogi.KING - 1].item()),
            OWN_PIECE_OFFSET + shogi.KING - 1,
        )
        self.assertEqual(
            int(features.square_feature_ids[relative_5e, 14 + shogi.PROM_ROOK - 1].item()),
            OPPONENT_PIECE_OFFSET + shogi.PROM_ROOK - 1,
        )
        self.assertEqual(
            int(features.square_feature_ids[relative_5i, 14 + shogi.KING - 1].item()),
            OPPONENT_PIECE_OFFSET + shogi.KING - 1,
        )

    def test_input_layer_builds_alpha_zero_like_sequence(self) -> None:
        features = stack_shogi_position_features(
            [
                shogi_alpha_zero_like_position_features_from_sfen(shogi.Board().sfen()),
                shogi_alpha_zero_like_position_features_from_sfen(shogi.Board().sfen()),
            ]
        )
        layer = ShogiAlphaZeroLikePositionInputLayer(embedding_dim=8)
    ...
    def test_attention_logit_bias_targets_square_pairs_only(self) -> None:
        ...
        embeddings = torch.zeros((2, SHOGI_ALPHA_ZERO_LIKE_ELEMENT_COUNT, 8))
        ...
        self.assertEqual(tuple(bias.shape), (2, SHOGI_ALPHA_ZERO_LIKE_ELEMENT_COUNT, SHOGI_ALPHA_ZERO_LIKE_ELEMENT_COUNT))
        self.assertEqual(
            float(
                bias[
                    0,
        ...
        self.assertEqual(float(bias[0, 0, SHOGI_ALPHA_ZERO_LIKE_SQUARE_ELEMENT_OFFSET].item()), 0.0)
