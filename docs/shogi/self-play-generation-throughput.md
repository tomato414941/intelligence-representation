...
- After removing per-child string tie-break conversion in selection and adding
  action-index conversion tables,
  `w16_c1_s256_b64_g16_bf16_search_hot_paths_4000ada_6vcpu` reached 8.41
  plies/sec on a 6 vCPU RTX 4000 Ada community Pod. This is not a strict
  before/after comparison with the 14 vCPU A4000 runs.
- Batch-decoding action-plane legal priors reduced output decode time on a
  comparable 14 vCPU RTX A4000 community Pod. `w16_c1_s256_b64_g16_bf16_batched_decode_a4000`
  reached 13.15 plies/sec, with output decode down to 14.26s.
- Building the action-plane legal mask from action counts reduced
  `output_feature_build` to 15.23s in
  `w16_c1_s256_b64_g16_bf16_tensor_mask_a4000`. The total plies/sec is not a
  strict comparison with `batched_decode_a4000` because this run used a smaller
  12 vCPU / 31 GiB Pod and had shorter games.
...
| `w16_c1_s256_b64_g16_bf16_batched_decode_a4000` | 2026-06-11 | shogi-minimal-split-global-position-action-plane-mcts256-full | RTX A4000 | 14 vCPU, 62 GiB | community | SE | $0.17/hr | runpod-torch-v280 / torch 2.8.0+cu128 | 16 | 16 | 1 | 256 | 64 | 63.08 | 64 | 98.57% | 199.8 | 243.18 | 13.15 | 36.68% | 47.00% | 355 MiB / 16376 MiB | 107.57% | 144.00% | 1145 MiB | Batch-decoded action-plane legal priors with padded gather and mask softmax. Full legal expansion, no top-k pruning. Phase totals: legal moves 42.94s, action-index 35.25s, selection 55.84s, expand 15.73s. Backend totals: model forward 94.30s, output decode 14.26s, output feature build 28.87s, position feature build 32.74s. |
| `w16_c1_s256_b64_g16_bf16_tensor_mask_a4000` | 2026-06-11 | shogi-minimal-split-global-position-action-plane-mcts256-full | RTX A4000 | 12 vCPU, 31 GiB | community | SK | $0.17/hr | runpod-torch-v280 / torch 2.8.0+cu128 | 16 | 16 | 1 | 256 | 64 | 60.56 | 64 | 94.63% | 144.8 | 196.90 | 11.77 | 32.48% | 45.00% | 358 MiB / 16376 MiB | 109.78% | 181.00% | 1147 MiB | Built the action-plane legal mask from action lengths and skipped turn decode when action indices are provided. Full legal expansion, no top-k pruning. Not directly comparable to `batched_decode_a4000`: smaller Pod and shorter games. Phase totals: legal moves 38.76s, action-index 32.03s, selection 45.09s, expand 11.99s. Backend totals: model forward 71.44s, output decode 11.84s, output feature build 15.23s, position feature build 32.65s. |
| `w16_c1_s256_b64_g16_bf16_search_hot_paths_4000ada_6vcpu` | 2026-06-10 | shogi-minimal-split-global-position-action-plane-mcts256-full | RTX 4000 Ada | 6 vCPU, 31 GiB | community | US | $0.20/hr | runpod-torch-v280 / torch 2.8.0+cu128 | 16 | 16 | 1 | 256 | 64 | 62.94 | 64 | 98.35% | 197.9 | 376.42 | 8.41 | 16.89% | 21.00% | 356 MiB / 20475 MiB | 103.90% | 150.00% | 1086 MiB | Removed per-child string tie-break conversion in selection and added action-index conversion tables. Full legal expansion, no top-k pruning. Not directly comparable to the 14 vCPU A4000 runs. Phase totals: legal moves 81.86s, action-index 69.88s, selection 99.27s, expand 27.11s. Backend totals: model forward 90.68s, output decode 89.91s, output feature build 24.90s, position feature build 57.37s. |
