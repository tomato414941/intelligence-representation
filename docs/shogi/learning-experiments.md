...
| --- | --- | --- | --- | --- | --- | --- |
| `shogi-learning-20260515-001` | 2026-05-15 | Does online experience replay improve the checkpoint? | 4 iterations; next-iteration checkpoint = iteration final; start checkpoint ID not recorded; legacy start path `models/d256-h1024-heads8-l6-shogi/checkpoint.pt`; self-play 64/iteration + checkpoint-vs-YaneuraOu 64/iteration with checkpoint black fixed | seeded replay; 512 generated games; 65,992 generated train examples | fixed eval worsened from 2.5003 to 10.9028; no head-to-head match | strength improvement undetermined |
...
| `shogi-learning-20260517-001` | 2026-05-17 | Does one epoch of full Qhapaq policy/value winner training improve the promoted checkpoint? | RunPod A100 80GB PCIe; PyTorch 2.8.0+cu128; initialized from the previous promoted checkpoint; 9,670 steps; batch 512; num_workers 8; policy loss weight 1.0; value loss weight 1.0; eval every 1,000 steps; keep last 3 step checkpoints | Qhapaq full tensor cache; 4,951,012 train examples; 262,133 eval examples; train eval capped at 65,536 examples; final eval used the full eval set | train loss 2.0753 -> 1.9431; train value loss 1.0151 -> 0.9064; eval loss 2.0960 -> 1.9751; eval value loss 1.0150 -> 0.9084; eval accuracy 0.4059 -> 0.4257; eval top-3 accuracy 0.6818; eval top-5 accuracy 0.7795; actual steps 9,670; job wall time 3,463.4s. Post-training player match beat the previous promoted checkpoint 14-1-1 with alternating sides, MCTS128, batch64, self-play move selection, no illegal moves, average 136.56 plies. The promoted checkpoint beat YaneuraOu MaterialLv1 `go nodes 1` 10-6, `go nodes 10` 9-7, and `go nodes 100` 11-5 in 16-game checks, then lost to `go nodes 1000` 37-61-2 over 100 games with alternating sides, MCTS128, batch64, evaluation move selection, no illegal moves, and average 96.47 plies. Same-checkpoint self-play with sampled move selection produced legal games with result 4-11-1 from player A's perspective and average 169.56 plies | Policy/value winner training improved fixed eval metrics, clearly beat the previous promoted checkpoint, took small-match wins from YaneuraOu MaterialLv1 nodes1, nodes10, and nodes100, and lost the 100-game match against nodes1000. Same-checkpoint self-play is a generation sanity check, not a strength comparison |
