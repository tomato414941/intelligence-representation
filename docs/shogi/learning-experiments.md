...
| `shogi-learning-20260517-001` | 2026-05-17 | Does one epoch of full Qhapaq policy/value winner training improve the promoted checkpoint? | RunPod A100 80GB PCIe; PyTorch 2.8.0+cu128; initialized from the previous promoted checkpoint; 9,670 steps; batch 512; num_workers 8; policy loss weight 1.0; value loss weight 1.0; eval every 1,000 steps; keep last 3 step checkpoints | Qhapaq full tensor cache; 4,951,012 train examples; 262,133 eval examples; train eval capped at 65,536 examples; final eval used the full eval set | train loss 2.0753 -> 1.9431; train value loss 1.0151 -> 0.9064; eval loss 2.0960 -> 1.9751; eval value loss 1.0150 -> 0.9084; eval accuracy 0.4059 -> 0.4257; eval top-3 accuracy 0.6818; eval top-5 accuracy 0.7795; actual steps 9,670; job wall time 3,463.4s. Post-training player match beat the previous promoted checkpoint 14-1-1 with alternating sides, MCTS128, batch64, self-play move selection, no illegal moves, average 136.56 plies. The promoted checkpoint also beat YaneuraOu MaterialLv1 `go nodes 1` 10-6 and `go nodes 10` 9-7 with alternating sides, MCTS128, batch64, evaluation move selection, no illegal moves, average 111.25 and 95.06 plies respectively. Same-checkpoint self-play with sampled move selection produced legal games with result 4-11-1 from player A's perspective and average 169.56 plies | Policy/value winner training improved fixed eval metrics, clearly beat the previous promoted checkpoint, and took small-match wins from YaneuraOu MaterialLv1 nodes1 and nodes10. Same-checkpoint self-play is a generation sanity check, not a strength comparison |
