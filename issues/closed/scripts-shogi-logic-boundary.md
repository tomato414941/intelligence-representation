...
`scripts/` is still small enough to manage, but shogi data/training-data-bundle/store logic is
...
- `scripts/create_shogi_training_data_bundle.py` now delegates its reusable behavior
  to `src/intrep/worlds/shogi/training_data_bundle.py`.
...
- Training Data Bundle creation cannot be reused outside one CLI
- future PyTorch `Sampler` or online replay work has to copy script logic
- Training Data Bundle and Experience Store behavior are harder to import from tests or
  other tools
...
- Shogi Training Data Bundle creation behavior is importable from package code. [x]
...
- Training Data Bundle creation behavior lives in `src/intrep/worlds/shogi/training_data_bundle.py`.
